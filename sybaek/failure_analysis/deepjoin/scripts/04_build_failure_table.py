...
def main():
    ...
    df["pair_cardinality_proportion"] = df.apply(
        lambda row: safe_min_max_ratio(row["query_unique_count"], row["candidate_unique_count"]),
        axis=1,
    )
...
def safe_min_max_ratio(left, right):
    ...
    return min(left, right) / denominator if denominator else 0.0
